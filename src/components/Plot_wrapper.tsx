...
const computeLinePoints = ([c1,c2]:[IDataGroup,IDataGroup]) => {
  ...
  const xy:[number,number][] = xses.map((x)=>[x, (x*w["w"][0]+w["b"])/(-w["w"][1])])
  console.log("margins: ", svm.margins(xy))
  return xy
}

const Plot_wrapper = ({ plot_data }: { plot_data: IData }) => {
